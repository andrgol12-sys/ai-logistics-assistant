...
async def log_incoming_updates(messages):
    """Log all incoming messages before handler dispatch."""
    for message in messages:
        user_id = message.from_user.id if message.from_user else "unknown"
        text = getattr(message, "text", None) or f"[{message.content_type}]"
        preview = text[:100] + ("..." if len(text) > 100 else "")
        logger.info(f"Incoming message from user {user_id}: {preview}")
...
async def setup_bot():
    ...
    try:
        ...
        bot.set_update_listener(log_incoming_updates)
        handler_names = [
            h["function"].__name__
            for h in bot.message_handlers
        ]
        logger.info(
            f"Handlers imported successfully ({len(bot.message_handlers)} handlers): "
