...
from services.stt import normalize_transcription_result
...
async def handle_voice_message(message: types.Message):
    ...
    try:
        ...
        if response.get("error"):
            await bot.send_message(
                message.chat.id,
                response.get("text", "❌ Не удалось обработать голосовое сообщение."),
                parse_mode="",
            )
            return

        transcription = normalize_transcription_result(
            response.get("transcription") or response.get("text")
        )
        if transcription:
            logger.info(
                f"Sending recognized text to user {user_id}: {transcription[:120]}"
