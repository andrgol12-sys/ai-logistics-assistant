...
from services.stt import transcribe_voice_message, normalize_transcription_result
...
async def route_text_request(
    ...
    try:
        ...
        if mode == BotMode.RAG:
            ...
            logger.info(f"Routing text request to RAG for user {user_id}")
...
async def route_voice_request(
    ...
    try:
        ...
        raw_transcription = await transcribe_voice_message(voice_path)
        transcription = normalize_transcription_result(raw_transcription)
        logger.info(
            f"User {user_id} voice transcription: {transcription[:120]}"
            f"{'...' if len(transcription) > 120 else ''}"
        )

        if not transcription:
            raise ValueError("Could not recognize speech in the voice message")
